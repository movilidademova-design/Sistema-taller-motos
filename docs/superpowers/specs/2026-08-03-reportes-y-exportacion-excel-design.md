...
**Decisión (corregida el 2026-08-03 tras la revisión de código, confirmada con el usuario):** en esta fase **no se modifica el esquema**. El reporte de Pagos filtra por `order.branchId` cuando el pago tiene orden asociada. Los pagos **sin orden quedan fuera de los reportes por sucursal** y aparecen únicamente cuando un ADMIN exporta el taller completo, marcados como "Sin sucursal" en esa columna.

La versión anterior de esta decisión decía lo contrario — incluirlos en el reporte de todas las sucursales, con el argumento de que es preferible que un pago aparezca de más a que desaparezca. La revisión mostró que ese razonamiento se equivoca de riesgo: un pago que falta en el reporte de una sede salta a la vista al cuadrar caja (falta un recibo), pero un pago repetido en el reporte de dos sedes solo se nota si alguien suma ambos reportes y los compara contra un total del taller — falla en silencio, que en un reporte de dinero es peor. Con la decisión actual, sumar los reportes de todas las sucursales no cuenta nada dos veces, y el ADMIN sigue viendo la totalidad.
...
**Nota de diseño — única excepción a "exporta lo que ves":** la regla general de la sección 2 es que el botón exporta exactamente lo filtrado en pantalla. El export de Clientes se aparta en un punto: la lista solo muestra clientes activos (no hay forma de ver los inactivos en la interfaz), mientras que el archivo los incluye a todos, distinguidos por la columna "Estado". Es deliberado — un export sirve para analizar el histórico completo, y la pantalla no tiene manera de representar ese caso — pero significa que el archivo puede traer más filas de las que el usuario ve. El botón lo advierte para que no sorprenda.
