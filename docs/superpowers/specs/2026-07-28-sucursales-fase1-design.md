...
**Nota de diseño — revertida (2026-07-30):** la nota anterior describe el comportamiento tal como se implementó originalmente en esta fase, pero ese comportamiento resultó ser un callejón sin salida real: la búsqueda de cédula durante el ingreso está filtrada por sucursal, así que un cliente registrado en otra sucursal del mismo tenant nunca aparecía en la búsqueda — pero intentar crearlo de nuevo chocaba con `@@unique([tenantId, documentId])` y era rechazado con un 409 ("ya existe en otra sucursal"). Resultado: no se podía encontrar NI crear. Un cliente es la misma persona sin importar en qué sucursal del taller aparezca, así que se corrigió para que `Client` sea una entidad compartida por todo el tenant, no aislada por sucursal: `ClientsService.findAll`/`findByDocumentId` dejaron de filtrar por `branchId`, y tanto `ClientsService.create` como `OrdersService.createOrReactivateClient`/`intake` ahora REUTILIZAN el cliente existente ante un choque de cédula, sin importar en qué sucursal se registró originalmente, en vez de rechazar con 409. `Motorcycle` (ligado al cliente) siguió el mismo criterio: `mustFindTenantMotorcycle` (en `orders.service.ts`) ya no filtra por sucursal, así que una moto registrada en otra sucursal del mismo cliente puede adjuntarse a una orden en la sucursal actual. `MotorcyclesService.findAll`/`create` se corrigieron igual — el listado de "Vehículos" ya no se filtra por sucursal, y crear una moto para un cliente registrado en otra sucursal ya no falla con "Cliente no encontrado" (era el mismo callejón sin salida, solo que en `motorcycles.service.ts` en vez de `clients.service.ts`). `branchId` en `Client`/`Motorcycle` pasa a significar simplemente "sucursal donde se registró por primera vez", no un límite de acceso. `Order` NO cambió — una reparación sigue atada a la sucursal física donde ocurre, y el listado/creación de órdenes sigue filtrado por `branchId` como antes.
