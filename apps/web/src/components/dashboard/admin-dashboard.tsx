...
import { RevenueAreaChart, OrdersPerDayChart, HorizontalBarChart } from './charts';
...
/** Panel de Administrador/Super Administrador — también usado por Visualizador,
 * con las tarjetas de facturación condicionadas a `reports.viewProfitability`. */
export function AdminDashboard() {
  const { user } = useAuth();
  const canSeeProfitability = user?.role !== 'VIEWER' || !!user?.permissions['reports.viewProfitability'];
      ...
      <div className="grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-5">
        ...
        {canSeeProfitability && (
          <>
            ...
            <StatCard label="Gastos del mes" value={formatCurrency(cards?.expensesMonth ?? 0)} icon={Receipt} accent="warning" />
          </>
        )}
        ...
        <StatCard label="Inventario bajo" value={cards?.lowStockCount ?? 0} icon={AlertTriangle} accent="warning" />
      </div>
